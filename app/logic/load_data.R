    ...
    tryCatch({
      ...
      fetch_api_data <- function(endpoint) {
        ...
        if (status_code(response) != 200) {
          message("Failed to access endpoint ", endpoint, ": status code ", status_code(response))
          return(NULL)
        }
      ...
      if (is.null(model_info)) {
        message("Failed to get model info from API. Continuing to fetch other endpoints...")
      ...
      message("- churnRate: ", if (!is.null(model_info$churnRate)) paste0(model_info$churnRate) else "NULL")
      ...
      if (!is.null(raw_data) && nrow(raw_data) > 0) {
        message("Successfully retrieved raw_data with ", nrow(raw_data), " rows")
      ...
      if (is.null(full_predictions) ||
            (total_customers > 0 &&
               !is.null(raw_data) &&
               nrow(full_predictions) < nrow(raw_data) * 0.2)) {

        message("all-predictions endpoint returned incomplete data. Trying predictions endpoint with limit=all...")

        # Try the regular predictions endpoint with limit=all
        full_predictions_alt <- fetch_api_data("/api/model/predictions?limit=all")

        if (!is.null(full_predictions_alt) &&
      ...
      if (is.null(data$raw_data)) {
        message("Using predictions as raw_data (some customers may be missing)")
      ...
      if (total_customers > 0) {
        ...
        if (actual_count < total_customers * 0.9) {
          message(
            ...
            "Expected about ", total_customers, " from /model/info, ",
            "but got only ", actual_count, " in raw_data"
          )
          ...
          message("API returned incomplete raw_data. Falling back to local file...")
          return(NULL)
        } else {
          ...
          if (actual_predictions < total_customers * 0.25) {
            message("WARNING: Predictions count is lower than expected (",
                    actual_predictions, " vs expected ~", round(total_customers * 0.3),
  ...
  if (!is.null(cache_path)) {
    ...
    tryCatch({
      ...
      if ("raw_data" %in% names(data)) {
        message("- raw_data dimensions: ", nrow(data$raw_data), " x ", ncol(data$raw_data))
        if (nrow(data$raw_data) > 0) {
          message("- First few IDs: ", paste(head(data$raw_data[, 1], 3), collapse = ", "))
        }
      } else {
        message("WARNING: raw_data not found in loaded RDS file")
        # Create raw_data from predictions if available
        if ("predictions" %in% names(data) && is.data.frame(data$predictions) && nrow(data$predictions) > 0) {
          message("Creating raw_data from predictions data")
          data$raw_data <- data$predictions
        } else {
          message("WARNING: Cannot create raw_data, no predictions data available")
        }
      }

      message("Data loaded successfully from local file.")
      message("- raw_data rows: ", if ("raw_data" %in% names(data)) nrow(data$raw_data) else "not found")
      message("- predictions rows: ", if ("predictions" %in% names(data)) nrow(data$predictions) else "not found")
